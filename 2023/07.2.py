...
class Hand:
	...
	def calculate_rank(cards):
		...
		for card in cards:
			if card == 'J':
				jokers += 1
			elif card not in cards_accumulator:
				cards_accumulator[card] = 1
			else:
				cards_accumulator[card] += 1
		cards_repetition = list(cards_accumulator.values())
		if len(cards_repetition) == 0:
			return Rank.FIVE_OF_A_KIND
		elif max(cards_repetition) >= 5 - jokers:
			return Rank.FIVE_OF_A_KIND
		elif max(cards_repetition) >= 4 - jokers:
			return Rank.FOUR_OF_A_KIND
		elif jokers == 1 and cards_repetition.count(2) == 2:
			return Rank.FULL_HOUSE
		elif 3 in cards_repetition and 2 in cards_repetition:
			return Rank.FULL_HOUSE
		elif max(cards_repetition) >= 3 - jokers:
			return Rank.THREE_OF_A_KIND
		elif cards_repetition.count(2) == 2 - jokers:
			return Rank.TWO_PAIR
		elif max(cards_repetition) >= 2 - jokers:
